...
public class Competition extends Swap {
	...
	/** */
	private Map<Pokemon,Double> scores = new HashMap<Pokemon,Double>();
	...
	public void execute(Pokemon p1, Pokemon p2) {
		if (p1.getTrainer() != p2.getTrainer()) {
			...
			// store the scores
			scores.put(p1, score1);
			scores.put(p2, score2);
		}
	}
	...
	public Map<Pokemon, Double> getScores() {
		return scores;
	}

	public void setScores(Map<Pokemon, Double> scores) {
		this.scores = scores;
	}
	
	public String toString(){
		return "Winner: " + getWinner() + " : " + scores.get(getWinner()) + 
				"\nLooser " + getLooser() + " with score: " + scores.get(getLooser()); 
	}
}
